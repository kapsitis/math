...
def matched_new_criteria(text):
    ...
    # "sadalīt" needs context
    has_split = re.search(r'sadalīt', text, re.IGNORECASE)
    # Must split into parts/shapes, not just "split into cells" (which is description) or "split cards into groups"
...
def process_file(filepath):
    ...
    # Split by problem to process individually
    ...
    for part in parts:
        ...
        if "subdomain:DOM_GridCut" in part:
            # Check text content (approximate, before <small>)
            ...
            if matched_old_criteria(text) and not matched_new_criteria(text):
                ...
                # Remove the tag
                # Expecting "* subdomain:DOM_GridCut" inside <small>
                # It might have a preceding newline
